...
float PeakRmsDSP::gainToDb(float linearGain)
{
    ...
    return juce::jlimit(ff360_labs::METER_FLOOR_DB, ff360_labs::METER_CEILING_DB, db);
}
...
MeterData PeakRmsDSP::processBlock(const juce::AudioBuffer<float>& buffer)
{
    ...
    if (numChannels > 0 && numSamples > 0)
    {
        ...
        data.peakL = gainToDb(juce::FloatVectorOperations::findMaximum(leftData, numSamples));
        data.rmsL = gainToDb(std::sqrt(buffer.getRMSLevel(0, 0, numSamples)));
        
        if (numChannels > 1)
        {
            auto* rightData = buffer.getReadPointer(1);
            data.peakR = gainToDb(juce::FloatVectorOperations::findMaximum(rightData, numSamples));
            data.rmsR = gainToDb(std::sqrt(buffer.getRMSLevel(1, 0, numSamples)));
        }
        else
        {
            // Mono
